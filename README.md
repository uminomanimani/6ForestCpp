...
### 实验设计
本实验复现了使用C++重写了[6Forest](https://github.com/Lab-ANT/6Forest)，分别使用了不同的方法对同一数据集进行IPv6地址扫描空间生成。
...
如果没有：
```bash
sh run_without_cmake.sh #为什么名字要取这么长呢，因为如果叫做run.sh，那我都有cmake了，还要多打6个字符，岂不是很没面子😅
```
